...
const { generateProjectContributionReportPdf } = require('../services/projectReportPdfService');
...
const createProject = async (req, res) => {
    try {
        const { module, title, members, abstract, expectedSize } = req.body;
        ...
        const requestedSize = Number(expectedSize) > 0 ? Number(expectedSize) : (Array.isArray(members) ? members.length + 1 : 0);
        if (!requestedSize || requestedSize < 2) {
            return res.status(400).json({ success: false, message: 'Expected team size is required.' });
        }
        ...
        const systemSettings = setting?.data || {};
        const moduleKey = String(module || '').toUpperCase();
        const moduleSpecificLimit = Number(systemSettings.moduleGroupSizes?.[moduleKey]);
        const globalLimit = Number(systemSettings.maxGroupSize) || 6;
        const effectiveLimit = Number.isFinite(moduleSpecificLimit) && moduleSpecificLimit >= 2 ? moduleSpecificLimit : globalLimit;

        if (requestedSize > effectiveLimit) {
            return res.status(400).json({
                success: false,
                message: `The selected module allows a maximum team size of ${effectiveLimit} members.`
            });
        }

        if (Array.isArray(members) && members.length !== requestedSize - 1) {
            return res.status(400).json({
                success: false,
                message: `Selected members must match the chosen team size. For a team size of ${requestedSize}, select ${requestedSize - 1} other members.`
            });
        }
        ...
        const requireApproval = systemSettings.requireManagerApproval !== false; // defaults to true if not set
    ...
    try {
        ...
        const projects = await prisma.project.findMany({
            ...
            include: {
                members: {
                    include: {
                        user: {
                            select: { id: true, name: true, email: true, indexNumber: true, github: true }
                        }
                    }
                },
                tasks: {
                ...
                githubRepos: {
                    select: { id: true, repoName: true, owner: true }
                }
            },
            orderBy: { createdAt: 'desc' }
        });
        ...
        const enhanced = projects.map(p => {
            ...
            return {
                ...
                totalTasks,
                doneTasks,
                hasGithub: p.githubRepos.length > 0
            };
        });

        res.status(200).json({ success: true, projects: enhanced });
    } catch (error) {
...
const getFairnessAnalyticsData = async (req, res) => {
    try {
...
const getMemberTracking = async (req, res) => {
    try {
        const { projectId } = req.params;
        const project = await prisma.project.findUnique({
            where: { id: projectId },
        ...
        const memberUsers = project.members
            ...
            .map((m) => ({
                ...
                githubLogin: extractGithubLogin(m.user.github),
                xpPoints: Number.isFinite(m.user.xpPoints) ? m.user.xpPoints : 0,
                badges: String(m.user.badges || '')
                    .split(',')
                    .map((b) => b.trim())
                    .filter(Boolean)
            }));
...
const getEngagementAnalyticsEndpoint = async (req, res) => {
    try {
        const { projectId } = req.params;
...
const getIntelligenceOverviewEndpoint = async (req, res) => {
    try {
        const { projectId } = req.params;
...
const getIntelligenceReportPdfEndpoint = async (req, res) => {
    try {
        const { projectId } = req.params;
        const payload = await generateProjectContributionReportPdf(res, projectId);
        if (!payload) {
            if (!res.headersSent) {
        ...
        console.error('Intelligence PDF report error:', error);
        if (!res.headersSent) {
...
const resolveRiskFlag = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        console.error('Resolve RiskFlag error:', error);
        res.status(500).json({ success: false, message: 'Server error resolving anomaly' });
    }
};

const getManagerGroups = async (req, res) => {
    try {
